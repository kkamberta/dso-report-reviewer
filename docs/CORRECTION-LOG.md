...
| 5 | 2026-07-01 | Patch/upgrade remediation produced prose only, so operators did not get a consistent technical command box or persisted command in downloaded outputs. | User correction while reviewing row 42 in the operator UI. | tool-output | Fixed in demo agent/API/UI and reviewer prompt: patch/upgrade findings can carry `remediation_commands`, render command boxes, and persist commands to markdown/XLSX outputs. |
